...
const Cart = () => {
  ...
  const checkoutClass = (
    <button
      ...
      className="w-full py-3 px-3 rounded-lg bg-[#1E1E1E] flex items-center justify-between bg-heading text-sm sm:text-base text-white focus:outline-none transition duration-300"
    >
      <span className="align-middle font-medium font-serif">
        Proceed To Checkout
      </span>
      <span className="rounded-lg font-bold font-serif py-2 px-3 bg-white text-[#1E1E1E]">
        {currency}
        {cartTotal.toFixed(2)}
      </span>
    </button>
